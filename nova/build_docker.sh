...
docker build . --build-arg PROJECT=nova --build-arg PROFILES="requirements qemu nova   neutron linuxbridge ceph cinder ceph glance ceph nova" --build-arg FROM=ubuntu:startcloud --tag nova:ubuntu --no-cache --rm
